...
Metrics tester:
  ...
  - Computes:
      * COCO caption metrics: BLEU-1..4, METEOR (optional), ROUGE_L, CIDEr
...
Notes:
  - Uses installed Python-3 pycocoevalcap (NOT local coco-caption repo)
  - --skip_meteor avoids slow/buggy METEOR; METEOR mismatches are common
    ...
    if skip_meteor:
        ...
        try:
            ...
            try:
                ...
                # Avoid noisy destructor issues if close() exists.
...
def main() -> None:
    ...
    ap.add_argument("--name", type=str, required=True, help="Pred filename stem, e.g. blip2_flan_t5_xl_swap_queries")
    ap.add_argument("--coco", action="store_true")
    ap.add_argument("--vqa", action="store_true")
    ap.add_argument("--skip_meteor", action="store_true", help="Skip METEOR (fast + avoids Java/jar issues)")
    args = ap.parse_args()
